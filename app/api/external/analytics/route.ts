...
export async function GET(request: Request) {
  ...
  const { valid, tenant_id, user_id, api_key_id } = await validateApiKey(apiKey);
  if (!valid || !tenant_id) {
    await logApiUsage({ api_key_id: api_key_id || '', endpoint: '/api/external/analytics', status: 401 });
  ...
  if (!tenant?.features?.analytics) {
    await logApiUsage({ api_key_id: api_key_id || '', endpoint: '/api/external/analytics', status: 403 });
  ...
  await logApiUsage({ api_key_id: api_key_id || '', endpoint: '/api/external/analytics', status: 200 });
