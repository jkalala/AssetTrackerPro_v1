...
export async function POST(request: Request) {
  ...
  const { data: member } = await supabase.from('team_members').select('role').eq('team_id', team_id).eq('user_id', user.id).single();
  ...
  // Insert invitation
  const { error, data: invitation } = await supabase.from('team_invitations').insert({
    team_id,
    email,
    role,
    token,
