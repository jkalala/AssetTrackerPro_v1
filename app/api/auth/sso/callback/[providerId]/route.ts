  ...
  try {
    ...
    if (providerError || !provider) {
      return NextResponse.redirect(
        new URL('/login?error=invalid_sso_provider', request.url)
      )
    }
    ...
    if (!result.success || !result.userInfo) {
      return NextResponse.redirect(
        new URL(`/login?error=${encodeURIComponent(result.error || 'SSO authentication failed')}`, request.url)
      )
    }
    ...
    return NextResponse.redirect(
      new URL('/login?error=sso_callback_error', request.url)
    )
  }
}
