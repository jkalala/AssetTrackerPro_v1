...
export async function POST(request: Request) {
  ...
  if (!user_id || !title || !body) return NextResponse.json({ error: "Missing fields" }, { status: 400 });

  // Get FCM token for user
  const supabase = await createClient();
  const { data, error } = await supabase.from("push_tokens").select("token").eq("user_id", user_id).single();
