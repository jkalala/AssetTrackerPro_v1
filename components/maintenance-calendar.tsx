...
export default function MaintenanceCalendar({ assetId, schedules, history }: MaintenanceCalendarProps) {
  // Flatten all dates (next_due for schedules, performed_at for history)
  const events = [
    ...schedules.map(s => ({ date: s.next_due, type: s.status === 'overdue' ? 'overdue' : 'due', label: s.type })),
    ...
    <div className="border rounded p-4 bg-white">
      ...
      <div className="grid grid-cols-7 gap-2">
        {['Sun','Mon','Tue','Wed','Thu','Fri','Sat'].map(d => (
          <div key={d} className="text-xs font-bold text-center">{d}</div>
        ))}
        {days.map(date => {
          const key = date.toISOString().slice(0,10)
          const evs = grouped[key] || []
          return (
            <div key={key} className="h-16 border rounded flex flex-col items-center justify-center bg-gray-50 relative">
              <span className="text-xs text-gray-700">{date.getDate()}</span>
              {evs.map((ev, i) => (
                <span key={i} className={
                  ev.type === 'overdue' ? 'text-red-600 text-xs font-bold' :
                  ev.type === 'due' ? 'text-yellow-600 text-xs font-bold' :
