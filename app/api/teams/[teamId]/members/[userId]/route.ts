...
export async function PATCH(request: Request, { params }: { params: { teamId: string, userId: string } }) {
  const supabase = await createClient();
  const { teamId, userId } = params;
  const { role } = await request.json();
  if (!teamId || !userId || !role) return NextResponse.json({ error: 'Missing teamId, userId, or role' }, { status: 400 });
  // Check current user
...
export async function DELETE(request: Request, { params }: { params: { teamId: string, userId: string } }) {
  const supabase = await createClient();
  const { teamId, userId } = params;
