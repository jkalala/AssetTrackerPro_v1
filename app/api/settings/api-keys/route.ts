...
export async function GET() {
  ...
  const { data, error } = await supabase.from('api_keys').select('id, key_name, created_at, is_active, user_id, permissions, revoked_at').eq('tenant_id', profile.tenant_id).order('created_at', { ascending: false });
