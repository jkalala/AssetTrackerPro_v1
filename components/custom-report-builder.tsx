...
export default function CustomReportBuilder({ report, onClose }: { report?: any, onClose: () => void }) {
          ...
          <div className="flex gap-2 flex-wrap">
            {FIELD_OPTIONS.map(opt => (
              <label key={opt.value} className="flex items-center gap-1">
                <input type="checkbox" checked={fields.includes(opt.value)} onChange={() => handleFieldChange(opt.value)} />
                {opt.label}
              </label>
            ))}
          </div>
        ...
        <div>
          ...
          <div className="flex gap-2 flex-wrap">
            ...
            <div>
              ...
              <input type="text" value={category} onChange={e => setCategory(e.target.value)} placeholder="e.g. IT" className="border rounded px-2 py-1" />
            </div>
            <div>
              <label className="block text-xs mb-0.5">Status</label>
              <input type="text" value={status} onChange={e => setStatus(e.target.value)} placeholder="e.g. active" className="border rounded px-2 py-1" />
            </div>
          </div>
        </div>
