...
export default function QRTemplateDesigner() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setShowForm(false)
      setEditId(null)
      setForm({ name: "", logoUrl: "", fields: ["asset_id", "name"], fontSize: 14, labelPosition: "below", qrSize: 120, is_default: false })
      setLogoFile(null)
      setPreviewUrl("")
      fetchTemplates()
    } catch (e: any) {
          ...
          {loading ? (
            ...
            <>
              <div className="mb-4 flex justify-between items-center">
                ...
                <Button onClick={() => setShowForm((v) => !v)}>{showForm ? "Cancel" : "New Template"}</Button>
              </div>
                  ...
                  <div key={tpl.id} className="flex items-center gap-4 border p-2 rounded">
                    <div className="flex-1">
                      <div className="font-semibold">
                        {tpl.name} {tpl.is_default && <span className="text-xs text-blue-600">(Default)</span>}
                      </div>
                      <div className="text-xs text-gray-500">Fields: {tpl.config.fields?.join(", ")}</div>
                    </div>
                    <Button size="sm" variant="outline" onClick={() => handleEdit(tpl)}>Edit</Button>
                    <Button size="sm" variant="outline" onClick={() => handleSetDefault(tpl.id)} disabled={tpl.is_default}>Set Default</Button>
                    <Button size="sm" variant="outline" onClick={() => handleDelete(tpl.id)}>Delete</Button>
                  </div>
                ))}
              </div>
              {showForm && (
                <form className="mt-6 space-y-4" onSubmit={handleSubmit}>
                  ...
                  <div>
                    ...
                    {previewUrl && <img src={previewUrl} alt="Logo preview" className="mt-2 h-12" />}
                  </div>
                  <div>
                    ...
                    <div className="flex gap-2 flex-wrap">
                      {FIELD_OPTIONS.map((opt) => (
                        <label key={opt.value} className="flex items-center gap-1">
                          <input type="checkbox" checked={form.fields.includes(opt.value)} onChange={() => handleFieldChange(opt.value)} />
                          {opt.label}
                        </label>
                      ))}
                    </div>
                  ...
                  <div className="flex gap-4">
                    <div>
                      ...
                      <Input type="number" name="fontSize" value={form.fontSize} onChange={handleInputChange} min={8} max={32} />
                    </div>
                    <div>
                      <label className="block text-sm font-medium mb-1">Label Position</label>
                      <Select value={form.labelPosition} onValueChange={(v) => setForm((f) => ({ ...f, labelPosition: v }))}>
                    ...
                    <div>
                      ...
                      <Input type="number" name="qrSize" value={form.qrSize} onChange={handleInputChange} min={80} max={300} />
                    </div>
                  </div>
              ...
              {showForm && (
                <div className="mt-6">
                  ...
                  <div style={{
                    ...
                    {form.logoUrl && <img src={form.logoUrl} alt="Logo" style={{ height: 32, marginBottom: 8 }} />}
                    <div style={{
                      width: form.qrSize,
                      height: form.qrSize,
                      background: "#f3f3f3",
