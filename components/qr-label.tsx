...
export default function QRLabel({ asset, templateConfig, qrCodeUrl }: QRLabelProps) {
  ...
  const labelText = fields?.map((f: string) => asset[f]).filter(Boolean).join(" | ")
  ...
  return (
    <div style={{
      ...
      {labelPosition === "above" && (
        <div style={{ fontSize, marginBottom: 8 }}>{labelText}</div>
      )}
      ...
      {labelPosition === "below" && (
        <div style={{ fontSize, marginTop: 8 }}>{labelText}</div>
      )}
    </div>
  )
} 