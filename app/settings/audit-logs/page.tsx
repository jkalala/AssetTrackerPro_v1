      ...
      <Card>
        ...
        <CardContent>
          <form onSubmit={handleFilterSubmit} className="flex flex-wrap gap-4 mb-4">
            <Input name="user_id" value={filters.user_id} onChange={handleFilterChange} placeholder="User ID" className="w-40" />
            <Input name="entity" value={filters.entity} onChange={handleFilterChange} placeholder="Entity" className="w-32" />
            <Input name="action" value={filters.action} onChange={handleFilterChange} placeholder="Action" className="w-32" />
            <Input name="date_from" value={filters.date_from} onChange={handleFilterChange} type="date" className="w-36" />
            <Input name="date_to" value={filters.date_to} onChange={handleFilterChange} type="date" className="w-36" />
                ...
                <tbody>
                  {logs.map(log => (
                    <tr key={log.id}>
                      ...
                      <td className="p-2 border"><pre className="whitespace-pre-wrap break-all">{JSON.stringify(log.details, null, 2)}</pre></td>
