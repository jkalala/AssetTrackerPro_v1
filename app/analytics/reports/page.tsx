          ...
          {loading ? (
            ...
            <>
              <div className="mb-4 flex justify-between items-center">
                ...
                <Button onClick={() => { setShowBuilder(true); setEditReport(null); }}>New Report</Button>
              </div>
                ...
                {reports.map((r) => (
                  <div key={r.id} className="flex items-center gap-4 border p-2 rounded">
                    <div className="flex-1">
                      ...
                      <div className="text-xs text-gray-500">Last updated: {new Date(r.updated_at).toLocaleString()}</div>
                    </div>
                    <Button size="sm" variant="outline" onClick={() => { setShowBuilder(true); setEditReport(r); }}>Edit</Button>
                    <Button size="sm" variant="outline" onClick={() => handleDelete(r.id)}>Delete</Button>
                    <Button size="sm" variant="default" onClick={() => handleRun(r)} disabled={runningReport === r.id}>{runningReport === r.id ? "Running..." : "Run"}</Button>
                  </div>
                ))}
              </div>
              {showBuilder && (
                <CustomReportBuilder
                  report={editReport}
                  onClose={() => { setShowBuilder(false); setEditReport(null); fetchReports(); }}
                />
              )}
                    ...
                    <div className="mb-4">
                      ...
                      <svg width={groupLabels.length * 60} height="120">
                        {groupLabels.map((label, i) => {
                          ...
                          return (
                            <g key={label}>
                              <rect x={i * 60 + 10} y={110 - barHeight} width="40" height={barHeight} fill="#2563eb" />
                              <text x={i * 60 + 30} y={115} textAnchor="middle" fontSize="10">{label}</text>
                              <text x={i * 60 + 30} y={110 - barHeight - 4} textAnchor="middle" fontSize="10">{count}</text>
                            </g>
                          )
                        })}
                      </svg>
                  ...
                  <div className="overflow-x-auto">
                    <table className="min-w-full border">
                      <thead>
                        <tr>
                          {resultsFields.map((f) => (
                            <th key={f} className="border px-2 py-1 text-xs text-gray-600 bg-gray-50">{f}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody>
                        {groupLabels.length > 0
                          ? groupedResults.flat().map((row, idx) => (
                              <tr key={idx} className="even:bg-gray-50">
                                {resultsFields.map((f) => (
                                  <td key={f} className="border px-2 py-1 text-xs">{String(row[f] ?? "")}</td>
                                ))}
                              </tr>
                            ))
                          : results.map((row, idx) => (
