...
export default function AdminPanelPage() {
  ...
  const [roleDialog, setRoleDialog] = useState<{ open: boolean, role: any | null }>({ open: false, role: null })
  ...
  const [editAssetDialog, setEditAssetDialog] = useState<{ open: boolean, asset: any | null }>({ open: false, asset: null })
  const [deleteAssetDialog, setDeleteAssetDialog] = useState<{ open: boolean, asset: any | null }>({ open: false, asset: null })
  const [assignAssetDialog, setAssignAssetDialog] = useState<{ open: boolean, asset: any | null }>({ open: false, asset: null })
  ...
  const [logDetailDialog, setLogDetailDialog] = useState<{ open: boolean, log: any | null }>({ open: false, log: null })
  const [logPage, setLogPage] = useState(1)
  const LOGS_PER_PAGE = 10
  const [teamSearch, setTeamSearch] = useState("")
  const [addTeamDialog, setAddTeamDialog] = useState(false)
  const [newTeamName, setNewTeamName] = useState("")
  const [removeTeamDialog, setRemoveTeamDialog] = useState<{ open: boolean, team: any | null }>({ open: false, team: null })
  const [teamMembersDialog, setTeamMembersDialog] = useState<{ open: boolean, team: any | null }>({ open: false, team: null })
  const [memberEmail, setMemberEmail] = useState("")
  const [memberRoleDialog, setMemberRoleDialog] = useState<{ open: boolean, member: any | null, team: any | null }>({ open: false, member: null, team: null })
  const [teamMembers, setTeamMembers] = useState<any[]>([])
  const [apiTab, setApiTab] = useState(false)
  const [apiKeys, setApiKeys] = useState<any[]>([])
  const [apiKeyDialog, setApiKeyDialog] = useState(false)
  const [newApiKeyName, setNewApiKeyName] = useState("")
  const [revokeApiKeyDialog, setRevokeApiKeyDialog] = useState<{ open: boolean, key: any | null }>({ open: false, key: null })
  const [copiedKeyId, setCopiedKeyId] = useState<string | null>(null)
  const [depreciationEditDialog, setDepreciationEditDialog] = useState<{ open: boolean, asset: any | null }>({ open: false, asset: null })
  ...
  const [chartDialog, setChartDialog] = useState<{ open: boolean, asset: any | null }>({ open: false, asset: null })
  ...
  const [reportFilters, setReportFilters] = useState<{ field: string; op: string; value: string }[]>([])
  ...
  const [deleteRoleDialog, setDeleteRoleDialog] = useState<{ open: boolean, role: any | null }>({ open: false, role: null })
  // Add a separate userDialog state for user dialogs
  const [userDialog, setUserDialog] = useState<{ open: boolean, user: any | null }>({ open: false, user: null })
  ...
  const sources: {
    ...
    assets: { label: "Assets", fields: ["name", "category", "status", "purchase_value", "created_at"] },
    ...
    if (tab === "apikeys") {
      (async () => {
        try {
          const { createClient } = await import("@/lib/supabase/client")
          const supabase = createClient()
          const { data: keys } = await supabase.from("api_keys").select("id, name, created_at, last_used_at, revoked, key")
          setApiKeys(keys || [])
        } catch {
  ...
  const mostActiveUserId = Object.entries(userActivity).sort((a, b) => b[1] as number - (a[1] as number))[0]?.[0]
  ...
  const handleDeactivate = async (userObj: any, deactivate: boolean) => {
    try {
      ...
      toast({ title: deactivate ? "User deactivated" : "User reactivated", description: userObj.email })
      setUserDialog({ open: false, user: userDialog.user })
      setProfile({ ...profile })
    } catch {
  ...
  const handleSaveAsset = async () => {
    try {
      const { createClient } = await import("@/lib/supabase/client")
      const supabase = createClient()
      await supabase.from("assets").update({ name: editAssetName, category: editAssetCategory, status: editAssetStatus }).eq("asset_id", editAssetDialog.asset.asset_id)
  ...
  const handleAssignAsset = async () => {
    try {
      ...
      const supabase = createClient()
      // Find user by email
      const { data: user } = await supabase.from("profiles").select("id").eq("email", assignUserEmail).single()
      if (!user) throw new Error("User not found")
      await supabase.from("assets").update({ assignee_id: user.id }).eq("asset_id", assignAssetDialog.asset.asset_id)
  ...
  const handleExportLogs = () => {
    ...
    const rows = filteredLogs.map((l: any) => [l.action, l.performed_by, l.created_at?.slice(0, 10)])
  ...
  const openTeamMembers = async (team: any) => {
    try {
      const { createClient } = await import("@/lib/supabase/client")
      const supabase = createClient()
      const { data: members } = await supabase.from("team_members").select("id, user_id, role, joined_at, profiles:profiles(full_name, email)").eq("team_id", team.id)
  ...
  const handleAddMember = async () => {
    try {
      ...
      const supabase = createClient()
      // Find user by email
      const { data: userProfile } = await supabase.from("profiles").select("id").eq("email", memberEmail).single()
      if (!userProfile) throw new Error("User not found")
      await supabase.from("team_members").insert({ team_id: teamMembersDialog.team.id, user_id: userProfile.id, role: "member" })
  ...
  const handleCreateApiKey = async () => {
    try {
      const { createClient } = await import("@/lib/supabase/client")
      const supabase = createClient()
      const { data: key } = await supabase.from("api_keys").insert({ name: newApiKeyName }).select().single()
  ...
  const handleSaveDepreciation = async () => {
    try {
      ...
      if (error) {
        ...
        toast({ title: "Error", description: error.message || "Failed to update depreciation", variant: "destructive" })
        return
      }
    ...
    if (report.groupBy) {
      ...
      if (report.aggType && report.aggField) {
        const aggRows = Object.entries(grouped).map(([k, group]) => {
          ...
          if (report.aggType === "sum") val = group.reduce((a: number, b: any) => a + Number(b[report.aggField]), 0)
          if (report.aggType === "avg") val = group.reduce((a: number, b: any) => a + Number(b[report.aggField]), 0) / group.length
          if (report.aggType === "min") val = Math.min(...group.map((b: any) => Number(b[report.aggField])))
          if (report.aggType === "max") val = Math.max(...group.map((b: any) => Number(b[report.aggField])))
          return { [report.groupBy]: k, [report.aggField]: val }
        })
        return aggRows
      }
      // No aggregation, just group
      return Object.entries(grouped).map(([k, group]) => ({ [report.groupBy]: k, count: group.length }))
    }
    // Aggregation (if not grouped)
    if (report.aggType && report.aggField) {
      let val = null
      if (report.aggType === "count") val = rows.length
      if (report.aggType === "sum") val = rows.reduce((a: number, b: any) => a + Number(b[report.aggField]), 0)
      if (report.aggType === "avg") val = rows.reduce((a: number, b: any) => a + Number(b[report.aggField]), 0) / rows.length
      if (report.aggType === "min") val = Math.min(...rows.map((b: any) => Number(b[report.aggField])))
      if (report.aggType === "max") val = Math.max(...rows.map((b: any) => Number(b[report.aggField])))
    ...
    try {
      ...
      if (editIdx !== null && customReports[editIdx]?.id) {
        ...
        const res = await fetch("/api/custom-reports", {
          ...
          body: JSON.stringify({ id: customReports[editIdx].id, name: report.name, config: report })
        })
        const json = await res.json()
        if (json.error) throw new Error(json.error)
        setCustomReports(customReports.map((r, i) => i === editIdx ? { ...json.data, config: report } : r))
        ...
        <CardContent>
          ...
          <Tabs value={tab} onValueChange={setTab} className="w-full">
            ...
            <TabsContent value="users">
              <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-4 gap-2">
                <Input
                  ...
                  className="max-w-xs"
                />
                <Button onClick={() => setInviteDialogOpen(true)} variant="outline">Invite User</Button>
              </div>
              <Table>
                <TableHeader>
                  <TableRow>
                ...
                <TableBody>
                  {filteredUsers.map((u: any) => (
                    <TableRow key={u.id}>
                      ...
                      <TableCell>
                        <select value={u.role_id || ""} onChange={async e => {
                          const newRoleId = e.target.value
                          // Update user role_id in backend
                          await fetch("/api/users", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: u.id, role_id: newRoleId }) })
                          setUsers(users.map(user => user.id === u.id ? { ...user, role_id: newRoleId } : user))
                      ...
                      <TableCell>
                        <Button size="sm" variant="secondary" onClick={() => setUserDialog({ open: true, user: u })}>Change Role</Button>
                        <Button size="sm" variant="secondary" className="ml-2" onClick={() => setDeactivateDialog({ open: true, user: u })}>{u.deactivated ? "Reactivate" : "Deactivate"}</Button>
                        <Button size="sm" variant="secondary" className="ml-2" onClick={() => setResetDialog({ open: true, user: u })}>Reset Password</Button>
                      </TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              ...
              {/* Change Role Dialog */}
              <Dialog open={userDialog.open} onOpenChange={open => setUserDialog({ open, user: userDialog.user })}>
                <DialogContent>
                  <DialogHeader>
                    <DialogTitle>Change User Role</DialogTitle>
                  </DialogHeader>
                  <div>Change role for <b>{userDialog.user?.email}</b>:</div>
                  <div className="flex gap-2 mt-2">
                    {['user', 'manager', 'admin', 'super_admin'].map(r => (
                      <Button key={r} variant={userDialog.user?.role === r ? "default" : "outline"} onClick={() => handleChangeRole(userDialog.user, r)}>{r}</Button>
                    ))}
                  </div>
                </DialogContent>
              </Dialog>
              ...
              <Dialog open={deactivateDialog.open} onOpenChange={open => setDeactivateDialog({ open, user: deactivateDialog.user })}>
                <DialogContent>
                  <DialogHeader>
                    <DialogTitle>{deactivateDialog.user?.deactivated ? "Reactivate" : "Deactivate"} User</DialogTitle>
                  </DialogHeader>
                  <div>Are you sure you want to {deactivateDialog.user?.deactivated ? "reactivate" : "deactivate"} <b>{deactivateDialog.user?.email}</b>?</div>
                  <DialogFooter>
                    <Button variant="destructive" onClick={() => handleDeactivate(deactivateDialog.user, !deactivateDialog.user?.deactivated)}>
              ...
              <Dialog open={resetDialog.open} onOpenChange={open => setResetDialog({ open, user: resetDialog.user })}>
                <DialogContent>
                  <DialogHeader>
                    <DialogTitle>Reset Password</DialogTitle>
                  </DialogHeader>
                  <div>Send password reset email to <b>{resetDialog.user?.email}</b>?</div>
                  <DialogFooter>
                    <Button onClick={() => handleResetPassword(resetDialog.user)}>Send Reset Email</Button>
                  </DialogFooter>
                </DialogContent>
              </Dialog>
            </TabsContent>
            <TabsContent value="assets">
                ...
                <TableBody>
                  {filteredAssets.map((a: any) => (
                    <TableRow key={a.asset_id}>
                      <TableCell>
                        <Button variant="ghost" size="icon" onClick={() => handleBulkSelect(a.asset_id)}>
                          {bulkSelected.includes(a.asset_id) ? <CheckSquare className="h-4 w-4 text-blue-600" /> : <Square className="h-4 w-4 text-gray-400" />}
                        </Button>
                      </TableCell>
                      ...
                      <TableCell>
                        <Button size="sm" variant="secondary" onClick={() => window.open(`/asset/${a.asset_id}`, '_blank')} title="Preview" aria-label="Preview asset">
                          <Eye className="h-4 w-4" />
                        </Button>
                        <Button size="sm" variant="secondary" className="ml-2" onClick={() => handleEditAsset(a)} title="Edit" aria-label="Edit asset">
                          <Pencil className="h-4 w-4" />
                        </Button>
                        <Button size="sm" variant="secondary" className="ml-2" onClick={() => setAssignAssetDialog({ open: true, asset: a })} title="Assign" aria-label="Assign asset">
                          <UserPlus className="h-4 w-4" />
                        </Button>
                        <Button size="sm" variant="destructive" className="ml-2" onClick={() => setDeleteAssetDialog({ open: true, asset: a })} title="Delete" aria-label="Delete asset">
              ...
              {/* Edit Asset Dialog */}
              <Dialog open={editAssetDialog.open} onOpenChange={open => setEditAssetDialog({ open, asset: editAssetDialog.asset })}>
                <DialogContent>
                  <DialogHeader>
                    <DialogTitle>Edit Asset</DialogTitle>
                  </DialogHeader>
                  <Input placeholder="Name" value={editAssetName} onChange={e => setEditAssetName(e.target.value)} className="mb-2" />
                  <Input placeholder="Category" value={editAssetCategory} onChange={e => setEditAssetCategory(e.target.value)} className="mb-2" />
                  <Input placeholder="Status" value={editAssetStatus} onChange={e => setEditAssetStatus(e.target.value)} className="mb-2" />
                  <DialogFooter>
              ...
              {/* Delete Asset Dialog */}
              <Dialog open={deleteAssetDialog.open} onOpenChange={open => setDeleteAssetDialog({ open, asset: deleteAssetDialog.asset })}>
                <DialogContent>
                  <DialogHeader>
                    <DialogTitle>Delete Asset</DialogTitle>
                  </DialogHeader>
                  <div>Are you sure you want to delete <b>{deleteAssetDialog.asset?.name}</b>?</div>
                  <DialogFooter>
                    <Button variant="destructive" onClick={() => handleDeleteAsset(deleteAssetDialog.asset)}>Delete</Button>
                  </DialogFooter>
                </DialogContent>
              </Dialog>
              {/* Assign Asset Dialog */}
              <Dialog open={assignAssetDialog.open} onOpenChange={open => setAssignAssetDialog({ open, asset: assignAssetDialog.asset })}>
                <DialogContent>
                  <DialogHeader>
                    <DialogTitle>Assign Asset</DialogTitle>
                  </DialogHeader>
                  <Input placeholder="User email" value={assignUserEmail} onChange={e => setAssignUserEmail(e.target.value)} />
                  <DialogFooter>
            ...
            <TabsContent value="logs">
              <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-4 gap-2">
                <Input
                  ...
                  className="max-w-xs"
                />
                <Button variant="outline" onClick={handleExportLogs}><Download className="h-4 w-4 mr-2" />Export CSV</Button>
              </div>
              <Table>
                <TableHeader>
                  <TableRow>
                ...
                <TableBody>
                  {paginatedLogs.map((l: any) => (
                    <TableRow key={l.id}>
                      ...
                      <TableCell>
                        <Button size="sm" variant="secondary" onClick={() => setLogDetailDialog({ open: true, log: l })}><Info className="h-4 w-4" /></Button>
                      </TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              ...
              <div className="flex justify-end items-center gap-2 mt-2">
                <Button size="sm" variant="outline" disabled={logPage === 1} onClick={() => setLogPage(p => Math.max(1, p - 1))}>Prev</Button>
                <span className="text-xs">Page {logPage} of {totalLogPages}</span>
                <Button size="sm" variant="outline" disabled={logPage === totalLogPages || totalLogPages === 0} onClick={() => setLogPage(p => Math.min(totalLogPages, p + 1))}>Next</Button>
              </div>
              {/* Log Detail Dialog */}
              <Dialog open={logDetailDialog.open} onOpenChange={open => setLogDetailDialog({ open, log: logDetailDialog.log })}>
                <DialogContent>
                  <DialogHeader>
                    <DialogTitle>Log Details</DialogTitle>
                  </DialogHeader>
                  <div className="mb-2"><b>Action:</b> {logDetailDialog.log?.action}</div>
                  <div className="mb-2"><b>By:</b> {logDetailDialog.log?.performed_by}</div>
                  <div className="mb-2"><b>Date:</b> {logDetailDialog.log?.created_at?.slice(0, 10)}</div>
                  {logDetailDialog.log && (
            ...
            <TabsContent value="teams">
              <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-4 gap-2">
                <Input
                  ...
                  className="max-w-xs"
                />
                <Button variant="outline" onClick={() => setAddTeamDialog(true)}><Plus className="h-4 w-4 mr-2" />Add Team</Button>
              </div>
              <Table>
                <TableHeader>
                  <TableRow>
                ...
                <TableBody>
                  {filteredTeams.map((t: any) => (
                    <TableRow key={t.id}>
                      ...
                      <TableCell>
                        <Button size="sm" variant="secondary" onClick={() => openTeamMembers(t)}><UsersIcon className="h-4 w-4" /></Button>
                      </TableCell>
                      <TableCell>
                        <Button size="sm" variant="destructive" onClick={() => setRemoveTeamDialog({ open: true, team: t })}><XIcon className="h-4 w-4" /></Button>
                      </TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              ...
              <Dialog open={addTeamDialog} onOpenChange={setAddTeamDialog}>
                <DialogContent>
                  ...
                  <DialogFooter>
                    <Button onClick={handleAddTeam} disabled={!newTeamName}>Add</Button>
                  </DialogFooter>
                </DialogContent>
              </Dialog>
              {/* Remove Team Dialog */}
              <Dialog open={removeTeamDialog.open} onOpenChange={open => setRemoveTeamDialog({ open, team: removeTeamDialog.team })}>
                <DialogContent>
                  <DialogHeader>
                    <DialogTitle>Remove Team</DialogTitle>
                  </DialogHeader>
                  <div>Are you sure you want to remove <b>{removeTeamDialog.team?.name}</b>?</div>
                  <DialogFooter>
                    <Button variant="destructive" onClick={() => handleRemoveTeam(removeTeamDialog.team)}>Remove</Button>
                  </DialogFooter>
                </DialogContent>
              </Dialog>
              {/* Team Members Dialog */}
              <Dialog open={teamMembersDialog.open} onOpenChange={open => setTeamMembersDialog({ open, team: teamMembersDialog.team })}>
                <DialogContent>
                  <DialogHeader>
                  ...
                  <div className="mb-2 flex gap-2">
                    ...
                    <Button onClick={handleAddMember} disabled={!memberEmail}>Add</Button>
                  </div>
                  <Table>
                    <TableHeader>
                      <TableRow>
                    ...
                    <TableBody>
                      {teamMembers.map((m: any) => (
                        <TableRow key={m.id}>
                          ...
                          <TableCell>
                            <Button size="sm" variant="secondary" onClick={() => setMemberRoleDialog({ open: true, member: m, team: teamMembersDialog.team })}>{m.role}</Button>
                          </TableCell>
                          <TableCell>{m.joined_at?.slice(0, 10)}</TableCell>
                          <TableCell>
                            <Button size="sm" variant="destructive" onClick={() => handleRemoveMember(m)}><XIcon className="h-4 w-4" /></Button>
                          </TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
              ...
              <Dialog open={memberRoleDialog.open} onOpenChange={open => setMemberRoleDialog({ open, member: memberRoleDialog.member, team: memberRoleDialog.team })}>
                <DialogContent>
                  <DialogHeader>
                    <DialogTitle>Change Member Role</DialogTitle>
                  </DialogHeader>
                  <div>Change role for <b>{memberRoleDialog.member?.profiles?.email}</b>:</div>
                  <div className="flex gap-2 mt-2">
                    {['member', 'admin'].map(r => (
                      <Button key={r} variant={memberRoleDialog.member?.role === r ? "default" : "outline"} onClick={() => handleChangeMemberRole(memberRoleDialog.member, r)}>{r}</Button>
                    ))}
                  </div>
                </DialogContent>
              </Dialog>
            ...
            <TabsContent value="apikeys">
              <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-4 gap-2">
                <Button variant="outline" onClick={() => setApiKeyDialog(true)}><Key className="h-4 w-4 mr-2" />Create API Key</Button>
              </div>
              <Table>
                <TableHeader>
                  <TableRow>
                ...
                <TableBody>
                  {apiKeys.map((k: any) => (
                    <TableRow key={k.id}>
                      ...
                      <TableCell>
                        <Button size="sm" variant="ghost" onClick={() => handleCopyApiKey(k)} disabled={k.revoked}>
                      ...
                      <TableCell>
                        {!k.revoked && (
                          <Button size="sm" variant="destructive" onClick={() => setRevokeApiKeyDialog({ open: true, key: k })}><Trash className="h-4 w-4" /> Revoke</Button>
                        )}
                      </TableCell>
                    </TableRow>
                  ))}
              ...
              <Dialog open={apiKeyDialog} onOpenChange={setApiKeyDialog}>
                <DialogContent>
                  ...
                  <Input placeholder="API Key name" value={newApiKeyName} onChange={e => setNewApiKeyName(e.target.value)} />
                  <DialogFooter>
                    <Button onClick={handleCreateApiKey} disabled={!newApiKeyName}>Create</Button>
                  </DialogFooter>
                </DialogContent>
              </Dialog>
              {/* Revoke API Key Dialog */}
              <Dialog open={revokeApiKeyDialog.open} onOpenChange={open => setRevokeApiKeyDialog({ open, key: revokeApiKeyDialog.key })}>
                <DialogContent>
                  <DialogHeader>
                    <DialogTitle>Revoke API Key</DialogTitle>
                  </DialogHeader>
                  <div>Are you sure you want to revoke <b>{revokeApiKeyDialog.key?.name}</b>?</div>
                  <DialogFooter>
                    <Button variant="destructive" onClick={() => handleRevokeApiKey(revokeApiKeyDialog.key)}>Revoke</Button>
                  </DialogFooter>
                </DialogContent>
              </Dialog>
            </TabsContent>
            <TabsContent value="depreciation">
                ...
                <TableBody>
                  {data.assets.map((a: any) => (
                    <TableRow key={a.asset_id}>
                      ...
                      <TableCell>
                        <Button size="sm" variant="secondary" onClick={() => openDepreciationEdit(a)}><BarChart3 className="h-4 w-4" /> Edit</Button>
                        <Button size="sm" variant="secondary" className="ml-2" onClick={() => setChartDialog({ open: true, asset: a })}>View Chart</Button>
                      </TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              ...
              <Dialog open={depreciationEditDialog.open} onOpenChange={open => setDepreciationEditDialog({ open, asset: depreciationEditDialog.asset })}>
                <DialogContent>
                  <DialogHeader>
                    <DialogTitle>Edit Depreciation</DialogTitle>
                  </DialogHeader>
                  <Input placeholder="Purchase Value" type="number" value={depValue} onChange={e => setDepValue(e.target.value)} className="mb-2" />
                  <Input placeholder="Purchase Date" type="date" value={depDate} onChange={e => setDepDate(e.target.value)} className="mb-2" />
                  <select value={depMethod} onChange={e => setDepMethod(e.target.value)} className="w-full mb-2 border rounded p-2">
                    <option value="straight_line">Straight Line</option>
                  </select>
                  <Input placeholder="Years" type="number" value={depYears} onChange={e => setDepYears(e.target.value)} className="mb-2" />
                  <Input placeholder="Salvage Value" type="number" value={depSalvage} onChange={e => setDepSalvage(e.target.value)} className="mb-2" />
                  <DialogFooter>
              ...
              <Dialog open={chartDialog.open} onOpenChange={open => setChartDialog({ open, asset: chartDialog.asset })}>
                <DialogContent>
                  <DialogHeader>
            ...
            <TabsContent value="customreports">
              <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-4 gap-2">
                <Button variant="outline" onClick={() => { setReportDialog(true); setEditReportIdx(null); setReportName(""); setReportSource("assets"); setReportFields([]); setReportFilter(""); setReportAgg(""); setReportChart("table"); }}><FileText className="h-4 w-4 mr-2" />New Report</Button>
              </div>
              <Table>
                <TableHeader>
                  <TableRow>
                ...
                <TableBody>
                  {customReports.map((r, idx) => (
                    <TableRow key={idx}>
                      ...
                      <TableCell>
                        <Button size="sm" variant="secondary" onClick={() => setRunReport(r)}>Run</Button>
                        <Button size="sm" variant="secondary" className="ml-2" onClick={() => { setEditReportIdx(idx); setReportDialog(true); setReportName(r.name); setReportSource(r.source); setReportFields(r.fields); setReportFilter(r.filter); setReportAgg(r.agg); setReportChart(r.chart); }}><Edit className="h-4 w-4" /></Button>
                        <Button size="sm" variant="destructive" className="ml-2" onClick={() => deleteReport(idx)}><Trash className="h-4 w-4" /></Button>
                      </TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              ...
              <Dialog open={reportDialog} onOpenChange={setReportDialog}>
                <DialogContent>
                  <DialogHeader>
                    <DialogTitle>{editReportIdx !== null ? "Edit" : "New"} Custom Report</DialogTitle>
                  </DialogHeader>
                  <Input placeholder="Report name" value={reportName} onChange={e => setReportName(e.target.value)} className="mb-2" />
                  <div className="mb-2">
                    <label>Source: </label>
                    <select value={reportSource} onChange={e => { setReportSource(e.target.value as keyof typeof sources); setReportFields([]); }} className="border rounded p-2 ml-2">
                  ...
                  <div className="mb-2">
                    ...
                    {sources[reportSource].fields.map((f: string) => (
                      <label key={f} className="ml-2">
                        <input type="checkbox" checked={reportFields.includes(f)} onChange={e => setReportFields(e.target.checked ? [...reportFields, f] : reportFields.filter(x => x !== f))} /> {f}
                      </label>
                    ))}
                  </div>
                  <div className="mb-2">
                    ...
                    {reportFilters.map((f, i) => (
                      <div key={i} className="flex gap-2 mb-1">
                        <select value={f.field} onChange={e => setReportFilters(reportFilters.map((x, j) => j === i ? { ...x, field: e.target.value } : x))}>
                          {sources[reportSource].fields.map((fld: string) => <option key={fld} value={fld}>{fld}</option>)}
                        </select>
                        <select value={f.op} onChange={e => setReportFilters(reportFilters.map((x, j) => j === i ? { ...x, op: e.target.value } : x))}>
                          <option value="=">=</option>
                        ...
                        {(() => {
                          ...
                          if (isDate && f.op === "between") {
                            ...
                            const range: DateRange = { from: start ? new Date(start) : undefined, to: end ? new Date(end) : undefined }
                            return (
                              <Calendar
                                mode="range"
                                selected={range}
                                onSelect={(range: DateRange | undefined) => {
                                  setReportFilters(reportFilters.map((x, j) =>
                                    j === i
                                      ? {
                                          ...x,
                                          value:
                            ...
                            return <Calendar mode="single" selected={f.value ? new Date(f.value) : undefined} onSelect={(d: Date | undefined) => setReportFilters(reportFilters.map((x, j) => j === i ? { ...x, value: d ? d.toISOString().slice(0,10) : "" } : x))} />
                          } else {
                            return <Input value={f.value} onChange={e => setReportFilters(reportFilters.map((x, j) => j === i ? { ...x, value: e.target.value } : x))} placeholder="Value" />
                          }
                        })()}
                        <Button size="sm" variant="destructive" onClick={() => setReportFilters(reportFilters.filter((_, j) => j !== i))}>Remove</Button>
                      </div>
                    ))}
                    <Button size="sm" variant="outline" onClick={() => setReportFilters([...reportFilters, { field: sources[reportSource].fields[0], op: "=", value: "" }])}>Add Filter</Button>
                  </div>
                  <div className="mb-2">
                    <label>Group By: </label>
                    <select value={reportGroupBy} onChange={e => setReportGroupBy(e.target.value)} className="border rounded p-2 ml-2">
                  ...
                  <div className="mb-2">
                    ...
                    <select value={reportAggType} onChange={e => setReportAggType(e.target.value)} className="border rounded p-2 ml-2">
                      <option value="">None</option>
                    ...
                    {reportAggType && reportAggType !== "count" && (
                      <select value={reportAggField} onChange={e => setReportAggField(e.target.value)} className="border rounded p-2 ml-2">
                        {sources[reportSource].fields.map((fld: string) => <option key={fld} value={fld}>{fld}</option>)}
                      </select>
                    )}
                  </div>
                  <DialogFooter>
                    <Button onClick={() => {
                      const report = { name: reportName, source: reportSource, fields: reportFields, filter: reportFilter, filters: reportFilters, agg: reportAgg, aggType: reportAggType, aggField: reportAggField, groupBy: reportGroupBy, chart: reportChart }
                      saveReport(report, editReportIdx)
                      setReportDialog(false)
                    }} disabled={!reportName || !reportFields.length || reportsLoading}>Save</Button>
                  </DialogFooter>
                  <div className="mt-4">
                    ...
                    <Table>
                      <TableHeader>
                        <TableRow>
                          {reportFields.map((f: string) => <TableHead key={f}>{f}</TableHead>)}
                        </TableRow>
                      </TableHeader>
                      <TableBody>
                        {getReportData({ name: reportName, source: reportSource, fields: reportFields, filter: reportFilter, filters: reportFilters, agg: reportAgg, aggType: reportAggType, aggField: reportAggField, groupBy: reportGroupBy, chart: reportChart }).slice(0, 5).map((row: any, i: number) => (
                          <TableRow key={i}>
                            {reportFields.map((f: string) => <TableCell key={f}>{row[f]}</TableCell>)}
                          </TableRow>
                        ))}
                      </TableBody>
                    </Table>
              ...
              {/* Run Report Dialog */}
              <Dialog open={!!runReport} onOpenChange={open => { if (!open) setRunReport(null) }}>
                <DialogContent>
                  <DialogHeader>
                  ...
                  {runReport && (
                    <>
                      <div className="mb-2 flex gap-2">
                        <Button size="sm" variant="outline" onClick={() => handleExportReport(getReportData(runReport))}>Export CSV</Button>
                      </div>
                      {runReport.chart === "table" && (
                        <Table>
                          <TableHeader>
                            <TableRow>
                              {runReport.fields.map((f: string) => <TableHead key={f}>{f}</TableHead>)}
                            </TableRow>
                          </TableHeader>
                          <TableBody>
                            {getReportData(runReport).map((row: any, i: number) => (
                              <TableRow key={i}>
                                {runReport.fields.map((f: string) => <TableCell key={f}>{row[f]}</TableCell>)}
                              </TableRow>
                            ))}
                          </TableBody>
                        </Table>
                      ...
                      {runReport.chart === "pie" && (
                        <ResponsiveContainer width="100%" height={300}>
                          <PieChart>
                            <Pie data={getReportData(runReport)} dataKey={runReport.fields[1]} nameKey={runReport.fields[0]} cx="50%" cy="50%" outerRadius={80} fill="#2563eb" label />
                            <Tooltip />
                          </PieChart>
                        </ResponsiveContainer>
                      )}
            ...
            <TabsContent value="roles">
              <div className="mb-4 flex flex-col md:flex-row md:items-center md:justify-between gap-2">
                <Button onClick={() => { setRoleDialog({ open: true, role: null }); setRoleName(""); setRolePerms([]) }}>Create Role</Button>
                ...
                <TableBody>
                  {roles.map(role => (
                    <TableRow key={role.id}>
                      ...
                      <TableCell>
                        <Button size="sm" variant="ghost" className="mr-2" onClick={() => { setViewPermsRole(role.name); setEditPerms(role.permissions) }}>View</Button>
                        {!role.is_builtin && <>
                          <Button size="sm" variant="outline" className="mr-2" onClick={() => { setRoleDialog({ open: true, role }); setRoleName(role.name); setRolePerms(role.permissions) }}>Edit</Button>
                          <Button size="sm" variant="destructive" onClick={() => setDeleteRoleDialog({ open: true, role })}>Delete</Button>
                        </>}
                      </TableCell>
                    </TableRow>
                  ))}
              ...
              <Dialog open={roleDialog.open} onOpenChange={open => setRoleDialog({ open, role: roleDialog.role })}>
                <DialogContent>
                  <DialogHeader>
                    <DialogTitle>{roleDialog.role ? "Edit Role" : "Create Role"}</DialogTitle>
                  </DialogHeader>
                  <Input placeholder="Role name" value={roleName} onChange={e => setRoleName(e.target.value)} className="mb-2" />
                  <div className="mb-2">Permissions:</div>
                  {ALL_PERMISSIONS.map(p => (
                    <label key={p} className="flex items-center gap-2 mb-1">
                      <input type="checkbox" checked={rolePerms.includes(p)} onChange={e => setRolePerms(e.target.checked ? [...rolePerms, p] : rolePerms.filter(x => x !== p))} />
                      {p}
                    </label>
                  ))}
                  <DialogFooter>
                    <Button onClick={saveRole} disabled={roleSaveLoading || !roleName}>{roleSaveLoading ? "Saving..." : "Save"}</Button>
                  </DialogFooter>
                </DialogContent>
              </Dialog>
              {/* Delete Role Dialog */}
              <Dialog open={deleteRoleDialog.open} onOpenChange={open => setDeleteRoleDialog({ open, role: deleteRoleDialog.role })}>
                <DialogContent>
                  <DialogHeader>
