...
export async function GET(request: Request) {
  ...
  if (!valid || !tenant_id) {
    await logApiUsage({ api_key_id: api_key_id || '', endpoint: '/api/external/users', status: 401 });
  ...
  if (error) {
    await logApiUsage({ api_key_id: api_key_id || '', endpoint: '/api/external/users', status: 500 });
